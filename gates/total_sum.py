"""게이트 5 — 총량 100%. 배합 %의 합이 100 ± 허용오차인가.
...
from gates.formulation import FormulationInput, GateResult  # type: ignore[import-not-found]
...
def check(fi: FormulationInput, *, tolerance: float = TOLERANCE, **_) -> GateResult:
    ...
    if nulls:
        return GateResult("게이트5 총량100%", "warning",
    ...
    if abs(total - 100.0) <= tolerance:
        return GateResult("게이트5 총량100%", "pass",
                          f"Σpct={total:.2f}% (100±{tolerance})",
                          provenance="규칙: Σpct=100±오차", hard=True)
    gap = total - 100.0
    return GateResult("게이트5 총량100%", "fail",
